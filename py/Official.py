...
class Chatbot:
    ...
    def reset(self, session) -> None:
        ...
        self.prompt.chat_histories[session] = []
        self.conversations.remove_conversation(session)
    ...
    def load_conversation(self, conversation_id) -> None:
        ...
        self.prompt.chat_history.clear()
...
def upload(prompt: str, session: str) -> str:
    try:
        prompt = prompt
        if prompt == "!reset":
            chatbot.reset(session)
            return ""
        chatbot.load_conversation(session)
    ...
    if not args.stream:
        ...
        for response in chatbot.ask_stream(prompt, temperature=args.temperature):
            ...
            sys.stdout.flush()
        print()
        chatbot.save_conversation(session)
        return res
